...
const calc_liquidityx96 = (low, cur, up, amt0, amt0_dec, amt1, amt1_dec) => {
  ...
  let amount_0 = Math.floor(amt0 * decimal); // Ensure it's an integer
  let amount_1 = Math.floor(amt1 * decimal); // Ensure it's an integer
  ...
  let liq = JSBI.BigInt(Math.min(Math.floor(liq0), Math.floor(liq1)));
  return liq;
};
...
const getPoolTicks = async (
  ...
  try {
        const ticks = await poolContract.ticks(tickLower);
        return ticks;
    } catch (error) {
...
const getTickFromPrice = (price, token0Decimals, token1Decimals) => {
  const base = 1.0001;
  const decimalDiff = parseInt(token0Decimals) - parseInt(token1Decimals);
  const priceInBase = price / Math.pow(base, decimalDiff);

  // Calculate the tick using logarithms with the base of 1.0001
  const tick = Math.log(priceInBase) / Math.log(base);
  return nearestUsableTick(tick, 60); // Assuming tickSpacing is 60 based on the usage in your code
};
...
const main = async () => {
  ...
  let tickLower = nearestUsableTick(Math.floor(tickLower_f), poolData.tickSpacing);
  let tickUpper_f = Math.log(upper/(10**decimalDiff)) / Math.log(base);
  let tickUpper = nearestUsableTick(Math.floor(tickUpper_f), poolData.tickSpacing);
  let {
    poolTicks,
    token0PriceChart,
    liquidityGross,
    volume24H
  } = await aprDataPreparation('500',tickLower,tickUpper,pool_contract_main,`ETH/USD`,WETH_ADDR,USDC_ADDR);
